...
@section('content')
  ...
  <form method="POST" action="{{ route('saas.onboarding.store') }}">@csrf
    <section class="panel"><div class="panel-heading"><div><h2>1. Estabelecimento</h2><p>Identificação principal do novo tenant.</p></div></div><div class="panel-body"><div class="form-grid">
      <label class="field"><span>Razão social</span><input name="clinic[corporate_name]" value="{{ old('clinic.corporate_name') }}" required></label>
      <label class="field"><span>Nome fantasia</span><input name="clinic[trade_name]" value="{{ old('clinic.trade_name') }}" required></label>
      ...
      <label class="field"><span>CNPJ</span><input name="clinic[cnpj]" value="{{ old('clinic.cnpj') }}"></label>
      <label class="field"><span>E-mail comercial</span><input type="email" name="clinic[email]" value="{{ old('clinic.email') }}"></label>
      <label class="field"><span>Telefone</span><input name="clinic[phone]" value="{{ old('clinic.phone') }}"></label>
      <label class="field"><span>WhatsApp</span><input name="clinic[whatsapp]" value="{{ old('clinic.whatsapp') }}"></label>
    ...
    <section class="panel"><div class="panel-heading"><div><h2>4. Administrador inicial</h2><p>Primeiro acesso do cliente, vinculado ao perfil Administrador.</p></div></div><div class="panel-body"><div class="form-grid">
      ...
      <label class="field"><span>Telefone</span><input name="admin[phone]" value="{{ old('admin.phone') }}"></label>
