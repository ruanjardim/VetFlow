...
@section('content')
  <header class="topbar">
    <div>
      ...
      <p>Do agendamento à retirada: confirme, faça o check-in, avise quando o animal estiver pronto e receba no PDV.</p>
    </div>
    <div class="actions">
      <a class="button secondary" href="{{ route('service-orders.agenda', ['date' => $date->toDateString()]) }}">Agenda do dia</a>
      ...
      <section class="service-order-column service-order-column--{{ $status }}">
        ...
        <div class="service-order-cards">
          @forelse($column['orders'] as $order)
            <article class="service-order-card {{ $order->isLate() ? 'is-late' : '' }}">
              ...
              @if($status === 'scheduled')
                <p><span class="badge {{ $order->isLate() ? 'warning' : ($order->status === 'confirmed' ? 'success' : 'muted-badge') }}">{{ $order->isLate() ? 'Atrasado' : $order->statusLabel() }}</span></p>
              @endif
              ...
              <div class="service-order-card-actions">
                @if($status === 'scheduled')
                  @if($order->status === 'scheduled')
                    <form method="POST" action="{{ route('service-orders.status', $order->id) }}">
                      ...
                      <button class="secondary" type="submit">Confirmar</button>
                    </form>
                  @endif
                  <form method="POST" action="{{ route('service-orders.status', $order->id) }}">
                    @csrf
                    ...
                    <button type="submit">Chegou (check-in)</button>
                  </form>
                  <form method="POST" action="{{ route('service-orders.status', $order->id) }}">
                    @csrf
                    @method('PATCH')
                    <input type="hidden" name="status" value="no_show">
                    <button class="secondary" type="submit">Não compareceu</button>
                  </form>
                @elseif($status === 'open')
                  <form method="POST" action="{{ route('service-orders.status', $order->id) }}">
                    @csrf
                ...
                @elseif($status === 'in_service')
                  <form method="POST" action="{{ route('service-orders.status', $order->id) }}">
                    ...
                    <button type="submit">Animal pronto</button>
                  </form>
