@php
  $statuses = \App\Modules\ServiceOrders\Models\ServiceOrder::STATUS_LABELS;
  $prefill = $prefill ?? [];
  $isEditing = isset($order) && $order;
  $defaultStatus = $isEditing ? $order->status : ($prefill['status'] ?? (! empty($prefill['scheduled_at']) ? 'scheduled' : 'open'));
  $scheduledValue = old('scheduled_at', $isEditing && $order->scheduled_at ? $order->scheduled_at->format('Y-m-d\TH:i') : ($prefill['scheduled_at'] ?? ''));
...
<div
  ...
  data-availability-url="{{ route('service-orders.availability') }}"
  data-order-id="{{ $isEditing ? $order->id : '' }}"
>
  ...
  <div class="field">
    <label for="scheduled_at">Data e hora do agendamento</label>
    <input id="scheduled_at" name="scheduled_at" type="datetime-local" value="{{ $scheduledValue }}" data-grooming-scheduled-at>
  </div>
  <div class="field">
    <label for="duration_minutes">Duração (min)</label>
    <input id="duration_minutes" name="duration_minutes" type="number" min="5" max="600" step="5" value="{{ old('duration_minutes', $order->duration_minutes ?? '') }}" placeholder="Soma dos serviços" data-grooming-duration>
  </div>
  <div class="field">
  ...
  <div class="field">
    ...
    <select id="tutor_id" name="tutor_id" data-service-order-tutor-select>
      ...
      @foreach($tutors as $tutor)
        <option value="{{ $tutor->id }}" data-clinic-id="{{ $tutor->clinic_id }}" @selected((int) old('tutor_id', $order->tutor_id ?? ($prefill['tutor_id'] ?? 0)) === $tutor->id)>{{ $tutor->name }}</option>
      @endforeach
    </select>
  </div>
  <div class="field">
    ...
    <select id="assigned_user_id" name="assigned_user_id" data-service-order-assigned-user-select>
      ...
      @foreach($assignedUsers as $assignedUser)
        <option
          ...
          @selected((string) old('assigned_user_id', $order->assigned_user_id ?? ($prefill['assigned_user_id'] ?? '')) === (string) $assignedUser->id)
        >{{ $assignedUser->name }}</option>
      @endforeach
    </select>
  </div>
  <div class="field full grooming-availability" data-grooming-availability hidden>
    <label>Horários livres do profissional</label>
    <div class="grooming-slot-chips" data-grooming-slots role="list"></div>
    <small class="muted" data-grooming-slots-status></small>
  </div>
  <div class="field">
    <label class="checkbox-inline">
      <input type="hidden" name="allow_overlap" value="0">
      <input type="checkbox" name="allow_overlap" value="1" @checked(old('allow_overlap'))>
      Encaixe (permitir horário já ocupado)
    </label>
  </div>
  @unless($isEditing)
    <div class="field">
      <label for="recurrence_frequency">Repetir agendamento</label>
      <select id="recurrence_frequency" name="recurrence_frequency" data-grooming-recurrence>
        <option value="">Não repetir</option>
        @foreach(($recurrenceFrequencies ?? []) as $frequencyValue => $frequency)
          <option value="{{ $frequencyValue }}" @selected(old('recurrence_frequency') === $frequencyValue)>{{ $frequency['label'] }}</option>
        @endforeach
      </select>
    </div>
    <div class="field">
      <label for="recurrence_count">Total de atendimentos</label>
      <input id="recurrence_count" name="recurrence_count" type="number" min="2" max="{{ config('petshop.grooming.max_recurrences', 12) }}" value="{{ old('recurrence_count', 4) }}" data-grooming-recurrence-count>
      <small class="muted">Inclui o primeiro. Ex.: 4 = pacote mensal de banho semanal.</small>
    </div>
  @endunless
                ...
                <select name="items[{{ $index }}][petshop_service_id]" data-service-order-service-select>
                  ...
                  @foreach($petShopServices as $petShopService)
                    <option
                      ...
                      data-duration="{{ $petShopService->duration_minutes }}"
